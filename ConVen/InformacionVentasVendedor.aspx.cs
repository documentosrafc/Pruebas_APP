...
public partial class Principal : System.Web.UI.Page
{
    ...
    {       
        ...
        com.Parameters.Add("@VENDEDOR", SqlDbType.VarChar, 10).Value = DropDownList3.Text.Trim();//Session["idvendedor"].ToString();
        data = new SqlDataAdapter(com);
        table = new DataTable();
        data.Fill(table);
    ...
    public void BuscarListadoClientes(string novendedor)
    {
        string buscar = @"SELECT LTRIM(A.ACCOUNTNUM) AS COD, LTRIM(A.NAME) AS NOMBRE,A.VATNUM AS RNC,A.PHONE AS TELEFONO, A.EMAIL,
                        ...
                        WHERE LTRIM(A.WPMSALESUNITID) NOT IN('058','400') AND LTRIM(A.WPMSALESUNITID)='" + DropDownList3.Text.Trim() + "' ORDER BY A.ACCOUNTNUM ";
    ...
    protected void Busc_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            string Buscarsaldo = @"SELECT LTRIM(C.ACCOUNTNUM) AS ID,LTRIM(C.NAME) AS NOMBRE, C.VATNUM AS RNC, B.INVOICE AS FACTURA, 
                                CONVERT(VARCHAR,A.TRANSDATE, 101) AS FECHA,
                                CONVERT(VARCHAR,CAST(A.AMOUNTCUR AS MONEY),1) AS MONTO_FAT,CONVERT(VARCHAR,CAST(A.AMOUNTMST AS MONEY),1) AS SALDO,
                                LTRIM(C.ADDRESS) AS DIRECCION, C.PHONE AS TELEFONO,LTRIM(C.WPMSALESUNITID) AS VEND
                                ...
                                INNER JOIN [10.0.0.54].AX50DOM_LIVE.DBO.CUSTINVOICEJOUR D ON B.ACCOUNTNUM=D.INVOICEACCOUNT AND B.INVOICE=D.INVOICEID
            ...
            if (table.Rows.Count > 0)
            {
                if (table.Rows[0]["VEND"].ToString().Trim() == DropDownList3.Text.Trim())
                {
                ...
                {
                    var a = "alert(\'El cliente que busca no pertenece a la cartera de su vendedor@\');";
                    ScriptManager.RegisterStartupScript(this, GetType(), "messageBox", a, true);
                }
            }
            else
    ...
    protected void DropDownList3_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (DropDownList3.Text != "Vendedores")
        {
            ...
            if (table.Rows.Count > 0)
            {
                ...
                BuscarListadoClientes(DropDownList3.Text.Trim());
            }
        }
    }
}