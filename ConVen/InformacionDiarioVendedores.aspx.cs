...
public partial class InformacionDiarioVendedores : System.Web.UI.Page
{
    ...
    private void GuardarAcopanamiento()
    {
        ...
        else
        {
            ...
            com.Parameters.Add("@COD_VENDEDOR", SqlDbType.VarChar, 20).Value = Session["idvendedor"].ToString();
            com.Parameters.Add("@COD_JEFA", SqlDbType.VarChar, 20).Value = Session["NOJEFA"].ToString();
            com.Parameters.Add("@RUTA_EFECTUADA", SqlDbType.VarChar, 60).Value = "0";
            ...
            com.Parameters.Add("@ACOMPANAMIENTO", SqlDbType.VarChar, 2).Value = "NO";
            com.UpdatedRowSource = UpdateRowSource.None;
            com.ExecuteNonQuery();


            string DEL = @"DELETE FROM Desarrollo.ConVen.ACOMPANAMIENTO_DET WHERE COD_VENDEDOR = '" + Session["idvendedor"].ToString() + "' AND FECHA = CONVERT(VARCHAR,GETDATE(),112)";
            ...
            {
                con.cn.Open();
                string DELCLIE = @"DELETE FROM Desarrollo.CONVEN.CLIENTES_NUEVOS_ACOMP WHERE COD_VENDEDOR='" + Session["idvendedor"].ToString() + "' AND FECHA=CONVERT(VARCHAR,GETDATE(),112)";
    ...
    public void cargarvisitasvendedores()
    {
        //aqui estoy llenando todos los clientes que estan asignado al vendedor seleccionado.
        string buscarclientes = @"SELECT A.ACCOUNTNUM AS CODIGO FROM [10.0.0.54].AX50DOM_LIVE.DBO.CUSTTABLE A WHERE A.WPMSALESUNITID='" + Session["idvendedor"].ToString() + "'";
        ...
        if (table.Rows.Count > 0)
        {
            ...
            string BUSCARACOMPANAMIENTOPENDIENTE = @"SELECT * FROM Desarrollo.ConVen.ACOMPANAMIENTO_CAB A 
                                                    ...
                                                    WHERE A.COD_VENDEDOR ='" + Session["idvendedor"].ToString() + "' AND A.FECHA_REGISTRO = CONVERT(VARCHAR,GETDATE(),112) AND A.ESTADO = 0";
            ...
            if (table.Rows.Count > 0)
            {
                ...
                string clientesnuevos = @"SELECT * FROM Desarrollo.ConVen.CLIENTES_NUEVOS_ACOMP WHERE COD_VENDEDOR='" + Session["idvendedor"].ToString() + "' AND FECHA=CONVERT(VARCHAR,GETDATE(),112)";
                ...
                if (table.Rows.Count > 0)
                {
                    Session["clientesnuevos"] = table.Rows.Count;
            ...
            else
            {
                ...
                string BUSCARACOMPANAMIENTOENVIADO = @"SELECT * FROM Desarrollo.ConVen.ACOMPANAMIENTO_CAB A                                                     
                                                    WHERE A.COD_VENDEDOR ='" + Session["idvendedor"].ToString() + "' AND A.FECHA_REGISTRO = CONVERT(VARCHAR,GETDATE(),112) AND A.ESTADO = 1";
    ...
    protected void DropDownList2_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (DropDownList2.Text != "Cliente Nuevo")
        {
            ...
            string buscarcliente = @"SELECT A.NAME AS NOMBRE, A.EMAIL AS CORREO, B.NUMBEROFEMPLOYEES AS OPERARIO, A.Dimension AS ACTIVIDAD
                                ...
                                ON A.PARTYID=B.PARTYID WHERE A.ACCOUNTNUM='" + DropDownList2.Text + "' AND A.WPMSALESUNITID='" + Session["idvendedor"].ToString() + "'";
    ...
    protected void subiranalisis_Click(object sender, EventArgs e)
    {
        ...
        if (DropDownList2.Text == "Clientes" || Label18.Text == string.Empty)
        {
    ...
    protected void Enviar_Click(object sender, EventArgs e)
    {
        ...
        {
            if (GridView1.Rows.Count <= 0 || int.Parse(novisita.Text) <= 0 || Label8.Text == string.Empty)
            {
            ...
            else
            {
                ...
                con.cn.Open();
                string ENVIARACOMP = @"UPDATE Desarrollo.ConVen.ACOMPANAMIENTO_CAB SET ESTADO = 1 WHERE COD_VENDEDOR='" + Session["idvendedor"].ToString() + "' AND FECHA_REGISTRO=CONVERT(VARCHAR,GETDATE(),112)";
                com = new SqlCommand(ENVIARACOMP, con.cn);
                com.UpdatedRowSource = UpdateRowSource.None;
                com.ExecuteNonQuery();
                con.cn.Close();

                var a = "alert(\'Trabajo diario envíado correctamente, gracias por usar nuestra plataforma\');";
                ScriptManager.RegisterStartupScript(this, GetType(), "messageBox", a, true);                                
            }
        }
    }
